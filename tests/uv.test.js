...
import { UV_BANDS, fetchUv, roundUv, uvBand } from '../src/uv.js';
import { STRINGS, resolveLang, t } from '../src/i18n.js';
...
describe('copy', () => {
    ...
    it('has a name and a line of advice for every band', () => {
        for (const band of UV_BANDS) {
            expect(t('band.' + band.id)).not.toBe('band.' + band.id);
            expect(t('advice.' + band.id)).not.toBe('advice.' + band.id);
        }
    });
});
